...
// Import Wordle's Answer List
$ukwords = file(__DIR__ . "/" . $_ENV['WRDANS_DICT']);

if (count($ukwords) > 0) {
    echo "<p>Found " . count($ukwords) . " words in Wordle Answer List</p>";
    echo "<p>";
    $i = 0;
    try {
        ...
        foreach ($ukwords as $key => $val) {
            ...
            try {
                $stmt = $dbh->prepare("INSERT INTO wordlist_srcans (word) VALUES (:word) ON DUPLICATE KEY UPDATE word=:word;");
    ...
    echo "<p>Could not load Wordle Answers</p>";
}
