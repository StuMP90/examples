...
// Import UK English words
$ukwords = file(__DIR__ . "/" . $_ENV['UK_DICT']);

if (count($ukwords) > 0) {
    echo "<p>Found " . count($ukwords) . " words in English Dictionary</p>";
    echo "<p>";
    $i = 0;
    try {
        ...
        foreach ($ukwords as $key => $val) {
            ...
            try {
                $stmt = $dbh->prepare("INSERT INTO wordlist_uk (word) VALUES (:word) ON DUPLICATE KEY UPDATE word=:word;");
    ...
    echo "<p>Could not load English Dictionary</p>";
}
