...
function pregen_kmz() {
    ...
    ?>
        <Placemark>
          ...
          <styleUrl>#arrowIcon<?= number_format(floor(abs($val[10])/22.5),0) ?></styleUrl>
          <description>
            <![CDATA[
    ...
    ?>
              ...
              Heading: <?= number_format(abs($val[10]),0) ?> degrees<br />
    ...
    // Save buffer into string and clean buffer
    $contents = ob_get_contents();
    ob_end_clean();

    // Generate compressesed .kml "file" and output it
    $tempkml = tempnam(__DIR__ . '/kmltemp', 'KML');
    $tempkmz = tempnam(__DIR__ . '/kmltemp', 'KML');
    $tempkmz_file = basename($tempkmz);
    $zip = new ZipArchive;
    file_put_contents($tempkml,$contents);
    $res = $zip->open($tempkmz, ZipArchive::CREATE | ZipArchive::OVERWRITE);
    if ($res === TRUE) {
        $zip->addFile($tempkml, "doc.kml");
        $zip->close();
        unlink($tempkml);
    }
    return $tempkmz_file;
